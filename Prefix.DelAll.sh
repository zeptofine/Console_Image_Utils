#!/bin/bash\
   ...
   while getopts ":hs:i:" opt; do
      case $opt in
         h) echo ""; help=1; break;;
         s) sort=$OPTARG; if [[ $sort = 1 ]]; then echo "sorting by age";elif [[ $sort = 2 ]]; then echo "sorting by name";files ;fi ;;
         i) folder="$OPTARG"; echo "Folder=$folder";;
...
# echo help dialog
   ...
   if [[ -z $input ]]; then echo "Folder to copy from: "; read -r input; if [[ -z $input ]]; then echo "No input folder given, exiting..."; exit 1; fi; fi
   if [[ -z $substring ]]; then echo "Substring to look for: "; read -r substring; if [[ -z $substring ]]; then echo "No prefix given, exiting..."; exit 1; fi; fi
   cd "$input" || exit
   nameshort=${input%*/}
   ...
   if [[ $sort = 1 ]]; then 
         ...
         mapfile -t fileArray < <(find "$folder" -type f -printf "%p,%T+\n" -name "*$PREFIX*"| sort | cut -d, -f1 ) && mapfile -t ArrayAge < <(find "$folder" -type f -printf "%p,%T+\n" -name "*$PREFIX*"| sort | cut -d, -f2 ); file # create array of files and array of ages in order of name
      fi; IFS=$OLDIFS

white='\033[1;37m'; lightblue='\033[1;34m'; green='\033[1;32m'
