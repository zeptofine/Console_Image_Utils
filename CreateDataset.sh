...
IFS=$OLDIFS
rm ./HR.log
echo "|Timestamp          |index|width:height|" >> ./HR.log
for (( i=0; i<${#fileArray[@]}; i++ )); do 
    ...
    if [[ $filext =~ .jpg ]] || [[ $filext =~ .png ]] || [[ $filext =~ .jpeg ]] || [[ $filext =~ .PNG ]]; then
        ...
        echo -e "${ArrayAge[$i]%.*} |$(echo "$i" | sed -e :a -e 's/^.\{1,4\}$/_&/;ta')|$(echo "$imagewidth" | sed -e :a -e 's/^.\{1,4\}$/_&/;ta'):$(echo "$imageheight" | sed -e :a -e 's/^.\{1,4\}$/_&/;ta')|" >> ./HR.log
                        else 
